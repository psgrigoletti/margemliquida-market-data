"""Módulo para buscar dados da Carteira Global (https://api.carteiraglobal.com/)"""
import logging
...
class CarteiraGlobal:
    """Classe para buscar dados da Carteira Global (https://api.carteiraglobal.com/)"""
    ...
    def setar_token(self, token: str):
        """setar_token configurar o token de acesso.

        Args:
            token (str): token fornecido para acesso aos dados.
        """
        self.token = token
    ...
    def retonar_cotacoes_fechamento(self, tickers, data_inicial, data_final):
        ...
        for t in tickers:
            try:
                ...
                retorno = self.retornar_cotacoes(
                    t_sem_sa, data_inicial, data_final
                )
                ...
                logging.log(
                    logging.ERROR, f'Erro ao buscar cotações de {t_sem_sa}'
                )
    ...
    def retonar_dados_fiis(self, ticker):
        ...
        if req.status_code == 200:
            ...
            logging.log(
                logging.INFO,
                'Resposta diferente de HTTP 200: ' + str(req.status_code),
            )
            raise Exception(
                'Resposta diferente de HTTP 200: ' + str(req.status_code)
            )
    ...
    def retornar_dividendos(
        ...
        if buscar_dy:
            div['Close'] = self.retornar_cotacoes(
